...
namespace Toolbox.NETMF.Hardware
{
    ...
    public class RgbLedStrip
    {
        ...
        public RgbLedStrip(Chipsets Chipset, int LedCount, SPI.SPI_module SPI_Device, Cpu.Pin ChipSelect_Port, bool ChipSelect_ActiveState)
        {
            ...
            else if (Chipset == Chipsets.WS2801)
            {
            ...
            else if (Chipset == Chipsets.WS2812)
            {
                // Creates a new buffer
                this._Buffer = new byte[LedCount * 3];
        ...
        public void Write()
        {
...
#else
            // bits needed for 3 bytes
            var bits = new byte[4 * _Buffer.Length];
            byte[] bitsTable = new byte[] {0x88, 0x8C, 0xC8, 0xCC};
            //byte[] bitsTable = new byte[] { 0xCC, 0xCC, 0xCC, 0xCC };
            ...
            for (var i = 0; i < _Buffer.Length; i++)
            {
                ...
                {
                    bits[i * 4 + 3 - bit] = (byte)bitsTable[tmp & 0x3];
                    tmp >>= 2;
                }                
            }

            this._Conn.Write(bits);
#endif
        }
    }
}
